...
打开你的 Steam 安装目录（默认 `[DRIVE_ROOT]/Program Files (x86)\Steam`），
找到 `steam.cfg` 文件。
...
del /f /q "[DRIVE_ROOT]/Windows\System32\merged_steam.py" 2>nul
echo 清理完成
```
...
$steamPaths = @(
    "[DRIVE_ROOT]/Program Files (x86)\Steam\steam.cfg",
    "[DRIVE_ROOT]/Steam\steam.cfg",
    "[DRIVE_ROOT]/Steam\steam.cfg",
    "[DRIVE_ROOT]/Steam\steam.cfg"
)
...
$pyPath = "[DRIVE_ROOT]/Windows\System32\merged_steam.py"
if (Test-Path $pyPath) {
